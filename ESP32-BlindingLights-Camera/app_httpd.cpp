...
static esp_err_t capture_handler(httpd_req_t *req) {
  ...
  framesize_t prev_framesize = (framesize_t)sens->status.framesize;
  int prev_quality = sens->status.quality;

  sens->set_framesize(sens, DIARY_CAPTURE_FRAMESIZE);
  sens->set_quality(sens, DIARY_CAPTURE_JPEG_QUALITY);
  ...
  if (!fb) {
    log_e("Camera capture failed");
    sens->set_framesize(sens, prev_framesize);
    sens->set_quality(sens, prev_quality);
    httpd_resp_send_500(req);
    return ESP_FAIL;
  }
...
static esp_err_t stream_handler(httpd_req_t *req) {
  ...
  while (true) {
    ...
    frame_time /= 1000;
    /* Per-frame Serial logging adds huge MJPEG latency; only do this when log level is verbose. */
